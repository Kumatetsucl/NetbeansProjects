...
public class DaoViaje{
    ...
    public ArrayList<Viaje> Listar(){
        try {
            ...
            while (reg.next()) {                
                ...
                viaj.setConductor(new DaoConductor().Buscar2(reg.getInt("ID_CONDUCTOR")));
                viaj.setUsuario(new DaoUsuario().Buscar2(reg.getInt("ID_CLIENTE")));
                viaj.setVehiculo(new DaoVehiculo().Buscar2(reg.getInt("ID_VEHICULO")));
                ...
                viaj.setFechaDeViaje(reg.getDate("FECHA_VIAJE"));
                listado.add(viaj);
            }
            return listado;
    ...
    public Viaje Buscar(String viaje) {
        try {
    ...
    public boolean Grabar(Viaje viaje) {
        try {
            String sql = "insert into Viaje values(SEQ_VIAJE.NEXTVAL,?,?,?,?,?,?,?)";
            PreparedStatement pstm = cone.prepareCall(sql);
            pstm.setInt(1, viaje.getConductor().getIdConductor());
            pstm.setInt(2, viaje.getUsuario().getIdCliente());
            pstm.setInt(3, viaje.getVehiculo().getIdVehiculo());
            ...
            java.sql.Date fecha = new java.sql.Date(viaje.getFechaDeViaje().getTime());
    ...
    public Viaje Buscar2(int idViaje) {
        try {
            String sql="SELECT * FROM VIAJE WHERE ID_VIAJE=?";
            PreparedStatement pstm=cone.prepareCall(sql);
    ...
    public boolean modificar(Viaje viaje){
        try {
